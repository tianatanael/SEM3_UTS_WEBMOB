...
// Ambil data dari GET
$idjual = isset($_GET['id']) ? $_GET['id'] : '';

// Ambil data dari tabel cuti dan pegawai
$stmt2 = $conn->prepare("SELECT p.idjual, p.idbarang, p.tanggal, p.jumlah, p.hargasatuan, b.namabarang, b.merk FROM penjualan p JOIN barang b ON p.idbarang = b.idbarang WHERE p.idjual = ? LIMIT 1");
$stmt2->bind_param('s', $idjual);
$stmt2->execute();
$stmt2->bind_result($idjual, $idbarang, $tanggal, $jumlah, $hargasatuan, $namabarang, $merk);
$stmt2->fetch();
$stmt2->close();
...
$pdf->Cell(0, 10, 'Nomor : ' . $idjual, 0, 1, 'C');
$pdf->Ln(1);
...
// Dari & Sampai
$pdf->Cell($labelWidth, 10, 'Tanggal Penjualan', 0, 0, 'L'); // Kolom label Tgl/Waktu
...
// Output PDF
$pdf->Output('I', 'data_penjualan.pdf');
?>
